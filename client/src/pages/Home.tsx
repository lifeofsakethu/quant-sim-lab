...
const assets = [
  { symbol: "BTC", name: "Bitcoin", price: 113842, change: "+2.38%", tone: "mint" },
  { symbol: "ETH", name: "Ethereum", price: 4482, change: "+1.11%", tone: "mint" },
  { symbol: "SOL", name: "Solana", price: 232, change: "-0.66%", tone: "red" },
  { symbol: "AAPL", name: "Apple", price: 231, change: "+1.32%", tone: "mint" },
];
    ...
    <div className="layout">
      ...
      <main className="workspace">
        ...
        {mode === "simulator" && <>
          ...
          <section className="instrument-grid"><div className="panel market-panel"><div className="panel-head"><div><div className="eyebrow">MARKET BOARD</div><h2>Watchlist / 04</h2></div><button className="icon-button" onClick={() => toast.success("Market snapshot refreshed locally")}><RefreshCw size={16}/></button></div>{assets.map((asset) => <button key={asset.symbol} onClick={() => setActiveAsset(asset.symbol)} className={`asset-row ${activeAsset === asset.symbol ? "selected" : ""}`}><span className="asset-icon">{asset.symbol.slice(0, 2)}</span><span className="asset-name"><b>{asset.symbol}</b><small>{asset.name}</small></span><strong>${asset.price.toLocaleString()}</strong><span className={asset.tone === "mint" ? "positive" : "negative"}>{asset.change}</span><ChevronRight size={14}/></button>)}</div><div className="panel order-panel"><div className="eyebrow">{activeAsset} / PAPER ORDER</div><h2>Trade the exchange</h2><div className="order-price"><span>{activeAsset}/USD</span><strong>${(assets.find((a) => a.symbol === activeAsset)?.price || 0).toLocaleString()}</strong><span className="positive">+2.38%</span></div><div className="order-tabs"><button className="active">BUY</button><button>SELL</button></div><label>ORDER SIZE / USDT NOTIONAL<input defaultValue="250" /></label><div className="preset-row">{["25", "100", "250", "500"].map((v) => <button key={v} onClick={() => setSendAmount(v)}>{v} USDT</button>)}</div><button className="primary-button wide" onClick={() => { if (exchange < 10) return toast.error("Send USDT to the exchange first"); log(`Bought ${activeAsset} with paper USDT`); toast.success(`Paper ${activeAsset} order filled`); }}><ArrowUpRight size={16}/> Buy {activeAsset}</button></div></section>
